...
class SafStorage(private val context: Context) {
    ...
    private fun read(tree: Uri, id: String): Map<String, Any?> = query(tree, uri(tree, id)).firstOrNull()
        ?: throw StorageFailure("not_found", "文件或文件夹已移除，或存储设备不可用")

    private fun children(tree: Uri, id: String): List<Map<String, Any?>> =
        query(tree, DocumentsContract.buildChildDocumentsUriUsingTree(tree, id))

    private fun query(tree: Uri, target: Uri): List<Map<String, Any?>> {
    ...
    private fun uri(tree: Uri, id: String) = DocumentsContract.buildDocumentUriUsingTree(tree, id)
